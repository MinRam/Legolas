...
/* const define */
const skipConfigCheckArg = "--dont-check-config";
const skipConfigGeneratorArg = "--dont-generate-config";
const skipConfigUpdateArg = "--dont-upgrade-config";
const themeName = "legolas";
/* ------------ */
...
function checkConfig(hexo) {
    if (!process.argv.includes(skipConfigCheckArg)) {
        logger.info('=== Checking theme configurations ===');

        const siteCfgFile = path.join(hexo.base_dir, '_config.yml');
        const themeSiteCfg = path.join(hexo.base_dir, `_config.${themeName}.yml`);
        ...
        if (!process.argv.includes(skipConfigGeneratorArg)) {
            if (!themeCfgPaths.length) {
                ...
                logger.info(`To skip configuration generation, use "${skipConfigGeneratorArg}".`);
            }
        }

        let cfg = loadThemeConfig(hexo, themeCfgPaths);

        if (!process.argv.includes(skipConfigUpdateArg)) {
            ...
            if (cfg.version && migrator.isOudated(cfg.version)) {
                ...
                logger.info(`To skip the configuration upgrade, use "${skipConfigUpdateArg}".`);
        ...
        if (validation !== true) {
            ...
            logger.warn(`${themeName} may still run, but you will encounter unexcepted results.`);
...
module.exports = hexo => {
    try {
        ...
        logger.info(`You may use \'${skipConfigCheckArg}\' to skip configuration checking.`);
        process.exit(-1);
    }
};
