...
module.exports = class extends Component {
    render() {
        ...
        /* Find the post index in the category */
        var post_index = 0;
        var posts = page.categories.data[0].posts.sort('-date');
        for (;post_index < posts.length; ++post_index) {
            if (posts.data[post_index]._id == page._id) {
                break;
            }
        }
        var category_post = {
            prev: post_index + 1 < posts.length ?  {
                path: posts.data[post_index + 1].path,
                title: posts.data[post_index + 1].title
            } : null,
            next: post_index > 0 ? {
                path: posts.data[post_index - 1].path,
                title: posts.data[post_index - 1].title
        ...
        return <Fragment>
            ...
            {/* Post navigation */}
            {!index && (category_post.prev || category_post.next) ? <nav class="post-navigation mt-4 level is-mobile">
                {category_post.prev ? <div class="level-start">
                    <a class={`article-nav-prev level level-item${!category_post.prev ? ' is-hidden-mobile' : ''} link-muted`} href={url_for(category_post.prev.path)}>
                        <i class="level-item fas fa-chevron-left"></i>
                        <span class="level-item">{category_post.prev.title}</span>
                    </a>
                </div> : null}
                {category_post.next ? <div class="level-end">
                    <a class={`article-nav-next level level-item${!category_post.next ? ' is-hidden-mobile' : ''} link-muted`} href={url_for(category_post.next.path)}>
                        <span class="level-item">{category_post.next.title}</span>
